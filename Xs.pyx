...
# ========== VOICE (Termux API ছাড়া - শুধু espeak) ==========
def speak(message):
    try:
        if IS_ANDROID:
...
# ========== LINK OPEN (Termux API ছাড়া - am start দিয়ে) ==========
def open_admin_contact(admin_link, admin_number, user_key, is_renew=False):
    ...
    if IS_ANDROID:
        os.system(f'am start -a android.intent.action.VIEW -d "{wa_link}" 2>/dev/null')
