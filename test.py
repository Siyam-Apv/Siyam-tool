...
def auto_install_dependencies():
    """Auto install all dependencies without user intervention"""
    
    print(f"\n{Y}[!] Checking and installing dependencies...{RESET}")
    ...
    try:
        ...
        print(f"{G}[✓] Git already installed{RESET}")
    except:
        print(f"{Y}[!] Installing Git...{RESET}")
        os.system("pkg install git -y 2>/dev/null")
        print(f"{G}[✓] Git installed{RESET}")
    
    # Install python if not present
    try:
        subprocess.run(["python", "--version"], capture_output=True, check=True)
        print(f"{G}[✓] Python already installed{RESET}")
    except:
        print(f"{Y}[!] Installing Python...{RESET}")
        os.system("pkg install python -y 2>/dev/null")
        print(f"{G}[✓] Python installed{RESET}")
    
    # Install python-cryptography (fast via pkg)
    try:
        from cryptography.fernet import Fernet
        print(f"{G}[✓] cryptography already installed{RESET}")
    except:
        print(f"{Y}[!] Installing python-cryptography...{RESET}")
        os.system("pkg install python-cryptography -y 2>/dev/null")
        print(f"{G}[✓] cryptography installed{RESET}")
    
    # Install espeak for voice
    try:
        subprocess.run(["which", "espeak"], capture_output=True, check=True)
        print(f"{G}[✓] espeak already installed{RESET}")
    except:
        print(f"{Y}[!] Installing espeak...{RESET}")
        os.system("pkg install espeak -y 2>/dev/null")
        print(f"{G}[✓] espeak installed{RESET}")
    
    # Install pip packages (FIXED)
    print(f"{Y}[!] Installing Python packages...{RESET}")
    
    # First upgrade pip
    os.system("python -m pip install --upgrade pip -q 2>/dev/null")
    
    # Install required packages
    packages = ["requests", "bs4", "mechanize", "urllib3", "cryptography"]
    for pkg in packages:
        try:
            __import__(pkg if pkg != "bs4" else "bs4")
            print(f"{G}[✓] {pkg} already installed{RESET}")
        except:
            print(f"{Y}[!] Installing {pkg}...{RESET}")
            os.system(f"python -m pip install {pkg} -q 2>/dev/null")
            print(f"{G}[✓] {pkg} installed{RESET}")
    
    print(f"{G}[✓] All dependencies ready!{RESET}\n")
    time.sleep(1)
...
def check_storage_permission():
    ...
    try:
        test_file = "/sdcard/.storage_test"
        with open(test_file, 'w') as f:
            f.write("test")
        os.remove(test_file)
        print(f"{G}[✓]{RESET} Storage permission granted")
        return True
    except:
        print(f"{R}[✗]{RESET} Storage permission NOT granted")
        print(f"{Y}[!]{RESET} Please run: termux-setup-storage")
        print(f"{Y}[!]{RESET} Then allow permission when popup appears")
        input(f"{G}[?] Press ENTER after allowing permission...{RESET}")
...
def main():
    ...
    # Then auto install dependencies
    ...
    while True:
        ...
        if key_valid:
            ...
            # Call your Facebook tool here
            print(f"{Y}[!]{RESET} Facebook Clone Tool Starting...{RESET}")
            # BNG_71_()  # Uncomment when you add this function
            return
        else:
            if key_expired:
